...
export class MetaService {
  ...
  constructor() {
    this.apiService = new ApiService(
      `${META_BASE_URL}/${META_BUSINESS_PHONE_ID}`
    );
  ...
  async downloadVideoAsBuffer(
    mediaId: string
  ): Promise<{ buffer: ArrayBuffer; mimeType: string }> {
    try {
      // Step 1: Get the download URL from WhatsApp API
      const mediaInfoResult = await this.graphApiService.get<{
        url: string;
        mime_type?: string;
      }>(`/${mediaId}`, {
        headers: {
      ...
      if (!mediaInfoResult.isSuccess || !mediaInfoResult.data) {
        throw new Error(
          `Failed to get media info: ${mediaInfoResult.error?.message || 'Unknown error'}`
        );
      }
      ...
      const downloadApiService = new ApiService(
        `${downloadUrlObj.protocol}//${downloadUrlObj.host}`
      );
      ...
      if (!videoResult.isSuccess || !videoResult.data) {
        throw new Error(
          `Failed to download video: ${videoResult.error?.message || 'Unknown error'}`
        );
      }
      ...
      logger.info(
        `✅ Video downloaded as buffer, size: ${videoBuffer.byteLength} bytes, mimeType: ${mimeType}`
      );
