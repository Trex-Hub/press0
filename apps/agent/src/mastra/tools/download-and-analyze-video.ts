...
export const downloadAndAnalyzeVideo = createTool({
  ...
  description:
    "Download and analyze a video that the user just sent via WhatsApp. The video is AUTOMATICALLY available via mediaId in runtime context. Simply call this tool with the user's question/prompt about the video. This tool is specifically for WhatsApp video messages.",
  ...
  execute: async ({ context, runtimeContext }) => {
    ...
    if (!mediaId) {
      throw new Error(
        'Missing mediaId - this tool requires a video from WhatsApp (mediaId in runtime context)'
      );
    }
    ...
    try {
      ...
      const { buffer, mimeType } =
        await metaService.downloadVideoAsBuffer(mediaId);
