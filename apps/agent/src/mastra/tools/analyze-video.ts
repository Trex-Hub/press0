...
export const analyzeVideo = createTool({
  id: 'analyze-video',
  description:
    'Analyze a video from a provided video URL. Use this tool when you have a direct video URL to analyze. For WhatsApp video messages, use download-and-analyze-video instead.',
  ...
  execute: async ({ context }) => {
    ...
    if (!videoUrl) {
      throw new Error(
        'Missing videoUrl - this tool requires a video URL to analyze'
      );
    }
